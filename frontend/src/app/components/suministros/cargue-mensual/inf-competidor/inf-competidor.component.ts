...
import { CargueCompetidorFila } from '../../../../models/cargue-mensual.models';
...
@Component({
  selector: 'app-inf-competidor',
  standalone: true,
  imports: [CommonModule, FormsModule, MessageModule, ...CommonPrimeNgModules, TablaAvanzadaComponent],
  templateUrl: './inf-competidor.component.html',
  styleUrl: './inf-competidor.component.css'
})
export class InfCompetidorComponent implements OnChanges {
  ...
  readonly filas = signal<CargueCompetidorFila[]>([]);
  ...
  descargarPlano(): void {
    window.open('assets/layout/planos/plano_competidor_mensual.csv', '_blank');
  }
  ...
  guardar(): void {
    ...
    this.confirmation.confirm({
      ...
      message: `¿Confirmás guardar ${this.filas().length} fila(s) de información de competidor para el período seleccionado?`,
  ...
  private confirmarGuardado(): void {
    ...
    this.service.setCargueInfCompetidor({
      aps: this.aps,
      ...
      next: () => {
        this.estado.set('Información de competidor guardada correctamente.');
        this.guardando.set(false);
        this.limpiarCargue();
        this.guardado.emit();
      },
      error: (e) => {
        this.estado.set(e?.error?.message || 'No fue posible guardar la información de competidor.');
        this.guardando.set(false);
      }
    });
  }
