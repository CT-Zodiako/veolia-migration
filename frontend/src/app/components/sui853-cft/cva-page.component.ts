...
// cva.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), mismo patrón que
...
@Component({
  ...
  styleUrl: './cva-page.component.css'
})
export class CvaPageComponent implements OnInit {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCvaSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCvaSeg3().subscribe({
      next: (res) => {
