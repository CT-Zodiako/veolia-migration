...
// crtsMaximo.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), mismo patrón que
// cft-page.component.ts.
@Component({
  selector: 'app-crts-maximo-page',
  standalone: true,
  imports: [CommonModule, ...CommonPrimeNgModules, Sui853Formato2TablaComponent],
  templateUrl: './crts-maximo-page.component.html',
  styleUrl: './crts-maximo-page.component.css'
})
export class CrtsMaximoPageComponent implements OnInit {
  ...
  private cargarSeg1(): void {
    ...
    this.service.getCrtsMaximo().subscribe({
      next: (res) => {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCrtsMaximoSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCrtsMaximoSeg3().subscribe({
      next: (res) => {
