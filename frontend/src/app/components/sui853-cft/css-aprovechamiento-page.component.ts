...
// cssAprovechamientoC.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), cada uno con su
// propio código de formato SUI (F853001/F853S201/F853S301). Ver
// doc migracion/modules/sui853/CFT.md.
@Component({
  selector: 'app-css-aprovechamiento-page',
  standalone: true,
  imports: [CommonModule, ...CommonPrimeNgModules, Sui853Formato2TablaComponent],
  templateUrl: './css-aprovechamiento-page.component.html',
  styleUrl: './css-aprovechamiento-page.component.css'
})
export class CssAprovechamientoPageComponent implements OnInit {
  ...
  private cargarSeg1(): void {
    ...
    this.service.getCssAprovechamiento().subscribe({
      next: (res) => {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCssAprovechamientoSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCssAprovechamientoSeg3().subscribe({
      next: (res) => {
