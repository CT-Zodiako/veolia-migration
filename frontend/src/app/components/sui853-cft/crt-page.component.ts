...
// crt.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), mismo patrón que
// cft-page.component.ts.
@Component({
  selector: 'app-crt-page',
  standalone: true,
  imports: [CommonModule, ...CommonPrimeNgModules, Sui853Formato2TablaComponent],
  templateUrl: './crt-page.component.html',
  styleUrl: './crt-page.component.css'
})
export class CrtPageComponent implements OnInit {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCrtSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCrtSeg3().subscribe({
      next: (res) => {
