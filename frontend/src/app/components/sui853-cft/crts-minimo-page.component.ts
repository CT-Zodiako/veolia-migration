...
// crtsMinimo.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), mismo patrón que
// cft-page.component.ts.
@Component({
  selector: 'app-crts-minimo-page',
  standalone: true,
  imports: [CommonModule, ...CommonPrimeNgModules, Sui853Formato2TablaComponent],
  templateUrl: './crts-minimo-page.component.html',
  styleUrl: './crts-minimo-page.component.css'
})
export class CrtsMinimoPageComponent implements OnInit {
  ...
  private cargarSeg1(): void {
    ...
    this.service.getCrtsMinimo().subscribe({
      next: (res) => {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCrtsMinimoSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCrtsMinimoSeg3().subscribe({
      next: (res) => {
