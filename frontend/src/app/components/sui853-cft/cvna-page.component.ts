...
// cvna.vue (legacy) — 3 tabs (SEG1/SEG2/SEG3), mismo patrón que
// cft-page.component.ts.
@Component({
  selector: 'app-cvna-page',
  standalone: true,
  imports: [CommonModule, ...CommonPrimeNgModules, Sui853Formato2TablaComponent],
  templateUrl: './cvna-page.component.html',
  styleUrl: './cvna-page.component.css'
})
export class CvnaPageComponent implements OnInit {
  ...
  private cargarSeg1(): void {
    ...
    this.service.getCvna().subscribe({
      next: (res) => {
  ...
  private cargarSeg2(): void {
    ...
    this.service.getCvnaSeg2().subscribe({
      next: (res) => {
  ...
  private cargarSeg3(): void {
    ...
    this.service.getCvnaSeg3().subscribe({
      next: (res) => {
