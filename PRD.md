...
# Problemas
Se va hacer un migracion de un sistema el cual esta en tecnologias viejas, y vamos hacer la migracion aqui en este pryoecto
...
- .net 10 para la api
- Dapper como orm
- Oracle 19 que tendra encuenta el sigueinte docker-compose ya que se esta haciendo todo en desarrolo aun
version: "3.9"
...
Vamos a utilizar mvc para todo ya que es facil de esguir eerrores y porqeu la aplicaion lo permite

tienes que hacer toda la configuracion dle pryecto 

# Como va a funcionar la migracion
LA migracion se hace atravez de archivos md que te van a suministra estos archvos traen toda la infomracion de como se dberia implentar el modulo a partir del flujo y funcioanlidades y creacion de front y back aqui tienes un ejemplo de como vendrna los md

Ejemplo de archivos como los vas a recibir para ir haciendo el codigo que se necesita

# Anexo A — Criterio de Paridad Funcional (AS-IS = TO-BE)

Este proyecto de migración tiene como objetivo **mantener exactamente el comportamiento actual** del módulo, cambiando la tecnología de implementación (frontend/backend), pero **sin introducir optimizaciones funcionales ni nuevas funcionalidades** en esta fase.

## Reglas de ejecución del alcance

1. **Paridad obligatoria**
   - Cada flujo documentado (F-AUTH-01 a F-AUTH-08) debe conservar el mismo resultado funcional observable en TO-BE.
   - Se debe mantener la misma lógica de negocio, validaciones y comportamiento esperado por el usuario final.

2. **Sin cambios de alcance funcional**
   - No se agregan features nuevas.
   - No se modifican procesos funcionales existentes.
   - No se altera UX por motivos de mejora (solo ajustes técnicos necesarios para migrar).

3. **Equivalencia de contrato API (AS-IS vs TO-BE)**
   - Se conserva el contrato funcional de endpoints: operación, entradas requeridas, códigos HTTP y forma de respuesta.
   - Cualquier variación técnica menor debe documentarse en el registro de desvíos controlados.

4. **Criterio de aceptación por equivalencia**
   - Para cada caso validado en AS-IS, su ejecución en TO-BE debe producir el mismo resultado funcional.
   - Si el resultado difiere, se considera no conforme hasta su corrección o aprobación formal del desvío.

5. **Registro de desvíos controlados (única excepción permitida)**
   - Toda diferencia entre AS-IS y TO-BE debe registrarse con:
     - ID del flujo afectado
     - Descripción del desvío
     - Motivo técnico
     - Impacto funcional
     - Aprobador responsable
   - Sin ese registro y aprobación, el cambio no se considera válido para esta fase.

## Definición de éxito de esta fase

- La migración se considera exitosa cuando los flujos funcionales actuales operan con paridad en la nueva tecnología.
- Mejoras, optimizaciones y nuevas capacidades quedan explícitamente fuera de alcance para una fase posterior.
...
- **db**:
  - `AUGE_SISTEMA`, `AUGE_MENU`, `AUGE_USUAMENU`, `SAUGE_USUAMENU`
- **trazabilidad**:
...
- `pendiente_ddl: TARIFICADOR.SAUGE_USUAMENU` (bitácora/histórico referenciado por naming, sin DDL entregado aún)
...
- `TARIFICADOR.SAUGE_USUAMENU`: `pendiente`
...
### ddl_pendiente
- `pendiente_ddl: TARIFICADOR.SAUGE_USUAMENU` — requerido para cerrar trazabilidad histórica/auditoría de asignaciones de menú.
