...
public interface IApsRepository
{
    ...
    Task<object?> CrearAsync(string nombre, string? idsui, int? resolucion, int propio, int relleno, int estado, int iat, long usuario, CancellationToken cancellationToken);
    Task<object?> EditarAsync(long id, string nombre, string? idsui, int? resolucion, int propio, int relleno, int estado, int iat, CancellationToken cancellationToken);
